...
df.insert(23, "RidgeClassifier_class", y_predict, True) 
df.insert(24, "RidgeClassifier_pro", y_predict, True) 
export_csv = df.to_csv ('./RidgeClassifier.csv', index = None)
print(confusion_matrix(ytest, y_predict))
tn, fp, fn, tp = confusion_matrix(ytest, y_predict, labels=[0,1]).ravel()
print(tn,fp,fn,tp)
with open('./split/result.csv', 'a') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['RidgeClassifier',tn,fp,fn,tp])
csvfile.close()
time = time.time() - start_time
with open('./split/time.csv', 'a') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['RidgeClassifier',time])
csvfile.close()

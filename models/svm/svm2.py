...
data = pd.read_csv("./feature_train_all.csv")
data = data.dropna()
...
svclassifier = SVC(kernel='poly',class_weight='balanced', degree=2, random_state=0)
...
datatest = pd.read_csv("./feature_test_all.csv")
...
df.insert(22, "svm2_class", y_predict, True) 
# df.insert(23, "svm6_pro", y_pro , True) 
export_csv = df.to_csv ('./svm2.csv', index = None)
print(confusion_matrix(ytest, y_predict))
tn, fp, fn, tp = confusion_matrix(ytest, y_predict, labels=[0,1]).ravel()
print(tn,fp,fn,tp)
with open('./result.csv', 'a') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['svm2',tn,fp,fn,tp])
csvfile.close()

time = time.time() - start_time
with open('./time.csv', 'a') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['svm2',time])
csvfile.close()

...
svclassifier = SVC(kernel='poly',class_weight='balanced', degree=3, random_state=0)
...
df.insert(23, "svm_linear_class", y_predict, True) 
df.insert(24, "svm_linear_pro", y_predict, True) 
export_csv = df.to_csv ('./output/svm_poly3.csv', index = None)
print(confusion_matrix(ytest, y_predict))
tn, fp, fn, tp = confusion_matrix(ytest, y_predict, labels=[0,1]).ravel()
print(tn,fp,fn,tp)
with open('./result.csv', 'a') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['svm_poly3',tn,fp,fn,tp])
csvfile.close()
time = time.time() - start_time
with open('./time.csv', 'a') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['svm_poly3',time])
csvfile.close()

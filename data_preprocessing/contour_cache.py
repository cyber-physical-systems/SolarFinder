...
def find_contours(img, mask_list):
    ...
    for i in range(0, masks_length):
        _,c, h = cv2.findContours(mask_list[i], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        for contour in c:
            cont.append(contour)
...
def main():
    # num = 0
    house_num = 8
#   initialize the haeader of contour features csv file
    csvpath = '/aul/homes/data/house' + str(house_num) + '/contour_features.csv'
    ...
    img_path = gb.glob('/aul/homes/dataset930/house' +str(house_num) + '/roof/*.png')
...
#             contour['cont'] = c
            ...
            csv_path = '/aul/homes/dataset930/house' + str(house_num)  + '/contourlabel.csv'
...
#                         num = num + 1
                        ...
                        cv2.imwrite(os.path.join('/aul/homes/data/house' + str(house_num)  + '/contour_all/' +contour['id'] + '.png'),img_result)
                        with open(csvpath, 'a') as csvfile:

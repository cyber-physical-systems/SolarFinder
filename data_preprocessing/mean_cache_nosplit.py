...
#     the file store the contour file
    csvpath_all = '/aul/homes/1019/nosplit/feature_train_all.csv'
    with open(csvpath_all, 'a') as csvfile:
        myFields = ['id','image', 'size','pole','mean','stddev','b_mean','g_mean','r_mean','b_stddev','g_stddev','r_stddev','square','ratiowh','ratioarea','approxlen','numangle','numangle90','numangle70','label','vgg_pro','vgg_class']
        writer = csv.DictWriter(csvfile, fieldnames=myFields)
        writer.writeheader()
    csvfile.close()
    # image path
    
    csv_path = '/aul/homes/1019/nosplit/feature_train.csv'
    with open(csv_path, newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            contour = row
            img_path_panel = '/aul/homes/final/nosplit/panel/' + contour['id'] +'.png'
            img_path_nopanel = '/aul/homes/final/nosplit/nopanel/' + contour['id'] +'.png'
            npy_path = '/aul/homes/dataset/dataset930/npy/'
            if (contour['label']=='1'):
                path = img_path_panel
            if (contour['label']=='0'):
            ...
            img = cv2.imread(path)
            c = np.load(npy_path + contour['id'] + '.npy')

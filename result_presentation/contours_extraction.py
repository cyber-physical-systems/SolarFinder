...
def filter_size(img, contour):
    ...
    # cv2.drawContours(img, a2, -1, (0, 0, 255), 2)
    cv2.imshow('img',img)
    cv2.waitKey(0)
    cv2.imwrite('./solar_panel/show/47.png',img)
    return [img, a2]
...
def main():
    img_path = gb.glob("./solar_panel/data/panel/3.png")
    ...
    for path in img_path:
        ...
        for i, area, c in contourinfo:
            ...
            mask = cv2.drawContours(img2gray, [c], 0, (255, 255, 255), -1)
            ...
            final = cv2.bitwise_or(img_result, bk)
<<<<<<< HEAD
            cv2.imwrite(('./solar_panel/show/' + str(i) + '.png'),final)
=======
